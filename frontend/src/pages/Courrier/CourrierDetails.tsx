...
import { usePiecesJointes, useAddPiecesJointes, useDeletePieceJointe, useDownloadPieceJointe } from "../../hooks/useCourriers";
...
const CourrierDetails: React.FC<CourrierDetailsProps> = ({ visible, onHide, courrier, onRefresh }) => {
  const [previewFile, setPreviewFile] = useState<{ url: string; name: string; type: string } | null>(null);
  const [uploading, setUploading] = useState(false);

  // Hooks pour les pièces jointes
  const { data: piecesJointes = [], refetch: refetchPieces, isLoading: loadingFiles } = usePiecesJointes(courrier?.id);
  ...
  const isTraitementEnRetard = (dateLimite: string, dateTraitement?: string) => {
  ...
  const getDelaiForAttribution = (attribution: any) => {
    ...
    const dateTraitement = attribution.date_traitement_effectif || courrier.date_traitement;
    const estTraite = attribution.est_traite || (dateTraitement && !isAfter(parseISO(dateTraitement), limite));
    ...
    if (estTraite && dateTraitement) {
      ...
      if (isAfter(traitementDate, limite)) {
        ...
        return { text: `Traitement en retard de ${retard}j`, severity: "danger" as const, isLate: true };
      }
      return { text: "Traité dans les délais", severity: "success" as const, isLate: false };
    }
    
    if (diff < 0) {
      return { text: `En retard de ${Math.abs(diff)}j`, severity: "danger" as const, isLate: true };
    }
    if (diff <= 2) {
      return { text: `${diff} jour${diff > 1 ? "s" : ""} restant${diff > 1 ? "s" : ""}`, severity: "warning" as const, isLate: false };
    }
    return { text: `${diff} jours restants`, severity: "success" as const, isLate: false };
  };
  ...
  const delaiInfo = (() => {
    if (courrier.date_limite_traitement) {
      ...
      const isLate = isTraitementEnRetard(courrier.date_limite_traitement, courrier.date_traitement);
    ...
    const attributionActive = courrier.attributions?.find((a: any) => a.est_active !== false);
    if (attributionActive?.date_limite_traitement) {
      const limite = parseISO(attributionActive.date_limite_traitement);
      const diff = differenceInDays(limite, new Date());
      const isLate = isTraitementEnRetard(attributionActive.date_limite_traitement, attributionActive.date_traitement_effectif);
  ...
  const openFilePreview = async (pj: any) => {
    const isImage = pj.type_fichier?.startsWith('image/') || 
                    pj.nom_fichier?.match(/\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i);
    const isPDF = pj.type_fichier === 'application/pdf' || 
                  pj.nom_fichier?.endsWith('.pdf');
    
    if (isImage || isPDF) {
      const url = pj.fichier_url?.startsWith('http') ? pj.fichier_url : `http://localhost:5001${pj.fichier_url}`;
      setPreviewFile({ url, name: pj.nom_fichier, type: isImage ? 'image' : 'pdf' });
    } else {
        ...
        <div className="pt-4 space-y-6 max-h-[80vh] overflow-y-auto px-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            {delaiInfo && (
              <div className={`bg-white border rounded-xl p-4 ${
                delaiInfo.isLate ? 'border-red-200 bg-red-50' : 'border-slate-200'
              }`}>
                <div className="text-xs text-slate-500 flex items-center gap-1">
                  <Clock size={12} />
                  Date limite / Délai
                ...
                {courrier.date_limite_traitement && (
                  <div className="text-xs text-slate-500 mt-2">
                    À traiter avant le: {formatDate(courrier.date_limite_traitement)}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {courrier.type === "ARRIVE" ? (
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                ...
                {courrier.expediteur_details?.adresse && (
                  <p className="text-xs text-slate-500 mt-1">{courrier.expediteur_details.adresse}</p>
                )}
              </div>
            ) : (
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                ...
                <p className="text-slate-700 font-medium">
                  {courrier.destinataire_externe?.nom || courrier.destinataire || "Non spécifié"}
                </p>
                {courrier.destinataire_externe?.adresse && (
                  <p className="text-xs text-slate-500 mt-1">{courrier.destinataire_externe.adresse}</p>
                )}
              </div>
            )}
            ...
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
              ...
              <div className="space-y-1 text-sm">
                <p><span className="text-slate-500">Création:</span> {formatDate(courrier.date_creation)}</p>
                <p><span className="text-slate-500">Réception:</span> {formatDate(courrier.date_reception)}</p>
                {/* NOUVEAU CHAMP: Date d'enregistrement */}
                <p className="text-emerald-700">
                  <span className="text-slate-500">📝 Enregistrement:</span> {formatDate(courrier.date_enregistrement)}
                </p>
                {courrier.date_attribution && <p><span className="text-slate-500">Attribution:</span> {formatDate(courrier.date_attribution)}</p>}
                {courrier.date_traitement && <p><span className="text-slate-500">Traitement:</span> {formatDate(courrier.date_traitement)}</p>}
                {courrier.date_limite_traitement && (
                  <p className={`font-medium ${isTraitementEnRetard(courrier.date_limite_traitement, courrier.date_traitement) ? 'text-red-600' : 'text-amber-600'}`}>
                    <span className="text-slate-500">📅 Date limite:</span> {formatDate(courrier.date_limite_traitement)}
                    {isTraitementEnRetard(courrier.date_limite_traitement, courrier.date_traitement) && (
                      <span className="ml-2 text-red-600 text-xs font-bold">⚠️ RETARD</span>
                    )}
                  </p>
                )}
              </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              ...
              <div className="relative">
                <Button
                  label={uploading ? "Upload en cours..." : "Ajouter des fichiers"}
                  icon={uploading ? <Loader2 size={16} className="animate-spin mr-2" /> : <Upload size={16} className="mr-2" />}
            ...
            {loadingFiles ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {piecesJointes.map((pj: any) => {
                  ...
                  return (
                    <div key={pj.idpiece_jointe} className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl p-3 hover:bg-white transition group">
                      <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => openFilePreview(pj)}>
                        <div className="w-8 h-8 bg-emerald-100 rounded-lg flex items-center justify-center">
                          {isImage ? '🖼️' : isPDF ? '📑' : '📄'}
                        </div>
                        <div className="flex-1">
                          <p className="text-sm font-medium truncate max-w-[180px]">{pj.nom_fichier}</p>
                          <p className="text-xs text-slate-400">
                            {pj.date_ajout ? new Date(pj.date_ajout).toLocaleDateString("fr-FR") : "Date inconnue"}
                          </p>
                        </div>
                      </div>
                      <div className="flex gap-1">
                        ...
                        <Button 
                          ...
                          className="p-button-text p-button-rounded text-slate-500 hover:text-emerald-600" 
                          onClick={() => handleDownloadFile(pj.idpiece_jointe, pj.nom_fichier)}
                          title="Télécharger"
                        />
                        <Button 
                          icon={<Trash2 size={16} />}
                          className="p-button-text p-button-rounded text-red-500 hover:text-red-700" 
                          onClick={() => confirmDeleteFile(pj.idpiece_jointe, pj.nom_fichier)}
                          title="Supprimer"
                        />
                      </div>
                    </div>
              ...
              <div className="text-center py-8 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <p className="text-slate-300 text-xs mt-1">Cliquez sur "Ajouter des fichiers" pour en ajouter</p>
              </div>
            )}
          </div>
          ...
          <div className="mt-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-3">
              <History size={14} className="text-emerald-500" /> Traçabilité complète
            </label>
            ...
            <div className="space-y-4">
                      ...
                      <div key={index} className="p-3 hover:bg-white transition">
                        <div className="flex items-start gap-3">
                          <div className="mt-0.5">
                            {audit.action === "CREATION" && <FilePlus size={14} className="text-green-600" />}
                            {audit.action === "VALIDATION" && <CheckCircle size={14} className="text-emerald-600" />}
                            {audit.action === "REJET" && <XCircle size={14} className="text-red-600" />}
                            {audit.action === "ATTRIBUTION" && <UserPlus size={14} className="text-blue-600" />}
                            {audit.action === "ATTRIBUTION_ENTITE" && <Building2 size={14} className="text-purple-600" />}
                            {audit.action === "TRAITEMENT" && <CheckCircle size={14} className="text-teal-600" />}
                            {audit.action === "TRANSFERT" && <RefreshCw size={14} className="text-orange-600" />}
                            {audit.action === "AJOUT_PIECE_JOINTE" && <Upload size={14} className="text-emerald-600" />}
                            {audit.action === "SUPPRESSION_PIECE_JOINTE" && <Trash2 size={14} className="text-red-600" />}
                          </div>
                          <div className="flex-1">
                            <div className="flex justify-between items-start flex-wrap gap-2">
                              ...
                              <span className="text-xs text-slate-400">
                                {new Date(audit.createdAt || audit.created_at).toLocaleString()}
                              </span>
                            </div>
                            <p className="text-xs text-slate-500 mt-1">{audit.details}</p>
                            {audit.agent && (
                        ...
                        <div key={index} className="p-3 hover:bg-white transition">
                          <div className="flex justify-between items-start flex-wrap gap-2">
                            <span className="text-sm font-medium text-slate-700">
                              Attribué à: {attrib.attribue_a?.nom} {attrib.attribue_a?.prenom || ""}
                            </span>
                            <span className="text-xs text-slate-400">
                          ...
                          {attrib.date_limite_traitement && (
                            <div className={`mt-2 p-2 rounded-lg ${
                              delaiAttrib?.isLate ? 'bg-red-50 border border-red-200' : 'bg-white border border-slate-200'
                            }`}>
                              <div className="flex justify-between items-center flex-wrap gap-2">
                                <span className="text-xs font-medium text-slate-600">
                                  ⏰ Date limite: {formatDate(attrib.date_limite_traitement)}
                                </span>
                                {delaiAttrib && (
                                  <Tag 
                          ...
                          {attrib.date_traitement_effectif && (
                            <p className="text-xs text-slate-500 mt-2">
                              ✅ Traité le: {formatDate(attrib.date_traitement_effectif)}
                            </p>
                          )}
                          ...
                          {attrib.attribue_par && (
                            <p className="text-xs text-slate-400 mt-1">
                              Attribué par: {attrib.attribue_par.nom} {attrib.attribue_par.prenom}
                            </p>
                          )}
                        </div>
                      );
              ...
              {/* Traitements */}
              {courrier.historique_traitements && courrier.historique_traitements.length > 0 && (
                <div className="bg-slate-50 rounded-xl border border-slate-100 overflow-hidden">
                  <div className="px-4 py-2 bg-slate-100 border-b border-slate-200">
                    <h4 className="text-sm font-bold text-slate-700 flex items-center gap-2">
                      <CheckCircle size={14} className="text-teal-600" />
                      Historique des traitements ({courrier.historique_traitements.length})
                    </h4>
                  </div>
                  <div className="divide-y divide-slate-100 max-h-64 overflow-y-auto">
                    {courrier.historique_traitements.map((traitement: any, index: number) => (
                      <div key={index} className="p-3 hover:bg-white transition">
                        <div className="flex justify-between items-start flex-wrap gap-2">
                          <span className="text-sm font-medium text-slate-700">
                            {traitement.action} {traitement.nouveau_statut && `→ ${traitement.nouveau_statut}`}
                          </span>
                          <span className="text-xs text-slate-400">
                        ...
                        {traitement.motif && (
                          <p className="text-xs text-slate-500 mt-1">📝 {traitement.motif}</p>
                        )}
                        {traitement.agent && (
                          <p className="text-xs text-slate-400 mt-1">
                            Par: {traitement.agent.nom} {traitement.agent.prenom}
                          </p>
                        )}
                      </div>
                    ))}
