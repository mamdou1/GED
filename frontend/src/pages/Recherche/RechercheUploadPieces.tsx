...
import { computePrefill } from "../Document/scanPrefill";
...
export default function RechercheUploadPieces({
  ...
  const [scanSuggestedKeys, setScanSuggestedKeys] = useState<
    Record<string, boolean>
  >({});
  const [scanAgentAvailable, setScanAgentAvailable] = useState<boolean | null>(
    null,
  );
  ...
  const handleScanPiece = async (pieceId: number) => {
    ...
    try {
      ...
      if (ocrText && ocrText.trim()) {
        ...
        updates.forEach((u) =>
          handleFormValueChange(pieceId, u.fieldId, u.value),
        );
        setScanSuggestedKeys((prev) => {
          const next = { ...prev };
          updates.forEach((u) => {
