// src/pages/destinataires/DestinatairesExternesDetails.tsx
...
interface DestinatairesExternesDetailsProps {
  visible: boolean;
  onHide: () => void;
  destinataire: DestinataireExterne | null;
  onEdit?: () => void;
}

const DestinatairesExternesDetails: React.FC<
  DestinatairesExternesDetailsProps
> = ({ visible, onHide, destinataire, onEdit }) => {
  if (!destinataire) return null;

  const isPersonne = destinataire.type === "PERSONNE";
  const displayName = isPersonne
    ? `${destinataire.nom || ""} ${destinataire.prenom || ""}`.trim()
    : destinataire.raison_sociale || "-";
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-xs text-slate-400 font-mono">
                ID: #{destinataire.iddestinataire_externe}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <p className="text-sm font-medium text-slate-700 mt-1">
                {formatDate(destinataire.createdAt)}
              </p>
            </div>
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
              <p className="text-[10px] font-bold text-slate-400 uppercase">
              ...
              <p className="text-sm font-medium text-slate-700 mt-1">
                {formatDate(destinataire.updatedAt)}
              </p>
            </div>
          </div>
        </div>
...
export default DestinatairesExternesDetails;
