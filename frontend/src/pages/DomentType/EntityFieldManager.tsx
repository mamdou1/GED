    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1 font-medium">
          {typeDocumentName || `Type #${typeDocumentId}`} • {entityType} :{" "}
          {entityName || `#${entityId}`}
        </p>
      </div>
    </div>
...
export default function EntityFieldManager({
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useEntityMetaFields(typeDocumentId, entityType, entityId);
  ...
  const handleAddCustomField = async () => {
    ...
    const technicalName =
      newField.name || newField.label.toLowerCase().replace(/[^a-z0-9]/g, "_");
      ...
      setOverride(
        {
          ...
          overrideData: {
            label_override:
              editForm.label_override !== editingField.label
                ? editForm.label_override
                : null,
            required_override:
              editForm.required_override !== editingField.required
                ? editForm.required_override
                : null,
      ...
      >
        <div className="mt-4 px-2">
          <TabView
            ...
            className="custom-tabview-field"
          >
            {/* Onglet 1 : Personnaliser les champs */}
            <TabPanel
              header="Personnaliser les champs"
              leftIcon={<Settings size={18} className="mr-2" />}
            >
              <div className="py-6">
                      ...
                      {fields.map((field) => (
                        ...
                        >
                          ...
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={() => handleEditOverride(field)}
                            ...
                            <button
                              ...
                              className={`p-2 rounded-lg ${field.hidden ? "text-emerald-600 hover:bg-emerald-50" : "text-amber-600 hover:bg-amber-50"}`}
                              title={
                                field.hidden ? "Rendre visible" : "Masquer"
                              }
                            >
                              {field.hidden ? (
                                <Eye size={18} />
                            ...
                            {field.source === "custom" && (
                              <button
                                onClick={() =>
                                  handleDeleteCustomField(field.id, field.label)
                                }
                                className="p-2 text-red-500 hover:bg-red-50 rounded-lg"
                                title="Supprimer"
                              >
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-3">
                        {customFields.map((field) => (
                          <div
                            key={field.id}
                            className="group flex justify-between items-center p-4 bg-purple-50/30 border border-purple-100 rounded-2xl"
                          >
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-1 flex-wrap">
                                  ...
                                  <span className="text-[9px] font-black bg-purple-100 text-purple-600 px-2 py-0.5 rounded">
                                    Personnalisé
                                  </span>
                                </div>
                                  ...
                                  <div className="flex flex-wrap gap-1 mt-2">
                                    {field.options
                                      ...
                                      .map((opt, idx) => (
                                        <span
                                          key={idx}
                                          className="text-[10px] bg-white text-slate-600 px-2 py-0.5 rounded border"
                                        >
                                          {opt}
                                        </span>
                                      ))}
                  ...
                  {!showAddForm ? (
                    ...
                    <div className="p-6 rounded-3xl border-2 border-purple-200 bg-purple-50/30">
                      <h4 className="text-[10px] font-black text-purple-600 uppercase tracking-widest mb-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              <InputText
                ...
                onChange={(e) =>
                  setEditForm({ ...editForm, label_override: e.target.value })
                }
                className="w-full p-3 border-slate-200 rounded-xl"
                placeholder={editingField.original?.label || editingField.label}
              />
              {editingField.original?.label && (
                <p className="text-xs text-slate-400 mt-1">
                  Original : {editingField.original.label}
                </p>
              )}
            </div>
            ...
            {editingField.source !== "custom" && (
              <div className="flex items-center gap-3 px-4 py-2 bg-white rounded-xl border">
                ...
                <InputSwitch
                  checked={editForm.required_override}
                  onChange={(e) =>
                    setEditForm({ ...editForm, required_override: e.value })
                  }
                />
              </div>
            )}
