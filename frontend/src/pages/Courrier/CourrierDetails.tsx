...
const CourrierDetails: React.FC<CourrierDetailsProps> = ({ visible, onHide, courrier }) => {
  ...
  const delaiInfo = (() => {
    ...
    if (diff < 0) return { text: `Retard de ${Math.abs(diff)}j`, severity: "danger" as const };
    if (diff <= 2) return { text: `${diff} jour${diff > 1 ? "s" : ""}`, severity: "warning" as const };
    return { text: `${diff} jour${diff > 1 ? "s" : ""}`, severity: "success" as const };
  })();
    ...
    >
      <div className="pt-4 space-y-6 max-h-[80vh] overflow-y-auto px-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          {delaiInfo && (
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              ...
              <Tag value={delaiInfo.text} severity={delaiInfo.severity} className="mt-1" />
            </div>
          )}
        </div>
                    ...
                    <div key={index} className="p-3 hover:bg-white transition">
                      <div className="flex justify-between items-start flex-wrap gap-2">
                        <span className="text-sm font-medium text-slate-700">
                          Attribué à: {attrib.attribue_a_agent?.nom} {attrib.attribue_a_agent?.prenom}
                        </span>
                        <span className="text-xs text-slate-400">
                      ...
                      {attrib.date_limite_traitement && (
                        <p className="text-xs text-slate-500 mt-1">
                          ⏰ Date limite: {new Date(attrib.date_limite_traitement).toLocaleString()}
                        </p>
                      )}
        ...
        {courrier.pieces_jointes && courrier.pieces_jointes.length > 0 && (
          <div>
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-3">
              <Upload size={14} className="text-emerald-500" /> Pièces jointes ({courrier.pieces_jointes.length})
            </label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {courrier.pieces_jointes.map((pj: any, index: number) => (
                <div key={index} className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl p-3 hover:bg-white transition">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-red-100 rounded-lg flex items-center justify-center">
                      📄
                    </div>
                    <div>
                      <p className="text-sm font-medium truncate max-w-[220px]">{pj.nom_fichier}</p>
